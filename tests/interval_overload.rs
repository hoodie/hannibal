...
async fn bounded_mailbox_with_slow_handler_skips_ticks() {
    ...
    let tick_tracker = hannibal::setup(TickTrackingActor {
...
async fn fast_handler_keeps_up_with_bounded_mailbox() {
    ...
    let addr = hannibal::setup(actor).bounded(3).spawn();
...
async fn demonstrates_generated_vs_received_with_sequence_numbers() {
    ...
    let addr = hannibal::setup(actor).bounded(2).spawn();
